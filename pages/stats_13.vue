...
export default {
    ...
    methods: { 
        ...
        generateBars() {
            ...
            var nested_data =  d3.rollups(data, v => d3.sum(v, d => d.extent) ,  d => d.subject).map(([k, v]) => ({ DateP: k, Sumextent: v }));
            ...
            x_scale.domain(dateNew.map((d) => d.subject));
            y_scale.domain([0, d3.max(dateNew, (d) => d.extent)]);
            ...
            .append("circle")
                .attr("fill", "blue")
                .attr("stroke", "none")
                .attr("cx", (d) => x_scale(d.subject) + x_scale.bandwidth()/2)
